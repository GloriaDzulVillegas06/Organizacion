...
const HALF_TIME_SECONDS = match => Math.max(1, Math.round((Number(match.duracion) || 40) * 60 / 2));
...
function render() {
  ...
  app.innerHTML = `<section class="match-page">
    ...
    <div class="timer" id="timer">${clock(match.elapsed || 0)}</div><div class="timer-controls"><button id="toggle-time">${ticking ? 'Ⅱ Pausar cronómetro' : match.phase === 'DESCANSO' ? '▶ Iniciar segundo tiempo' : '▶ Iniciar cronómetro'}</button></div>
    <section class="actions"><button class="action goal-us" id="goal-us">⚽ + GOL ${teamUpper()}</button><button class="action goal-them" id="goal-them">+ Gol rival</button><button class="action own-goal" id="own-goal">↺ Autogol</button><button class="action event" id="event">▣ Tarjeta / expulsión</button><button class="action break" id="break">${match.phase==='DESCANSO'?'Segundo tiempo':'Descanso'}</button><button class="action finish" id="finish">Finalizar partido</button></section>
...
function bind() {
  ...
  document.querySelector('#break').onclick = () => { if(match.phase === 'DESCANSO'){match.phase='SEGUNDO TIEMPO';ticking=false;saveRender(true)}else{match.phase='DESCANSO';saveRender(true)} };
...
async function toggleTimer() { const btn=document.querySelector('#toggle-time');if(match.phase==='DESCANSO'){match.phase='SEGUNDO TIEMPO';await matchesService.save(match)}if(!ticking&&match.estado!=='jugando'){btn.disabled=true;btn.textContent='Iniciando…';try{await matchesService.start(match.id);match.estado='jugando';matchesService.setActive(match);ticking=true;render();startClock();return}catch(error){btn.disabled=false;btn.textContent='▶ Iniciar cronómetro';alert(`No se pudo iniciar el partido: ${error.message}`);return}}ticking=!ticking;btn.textContent=ticking?'Ⅱ Pausar cronómetro':'▶ Continuar cronómetro';if(ticking)startClock();else clearInterval(timerId);render()}
function startClock(){clearInterval(timerId);timerId=setInterval(()=>{match.elapsed=(match.elapsed||0)+1;const timer=document.querySelector('#timer');if(timer)timer.textContent=clock(match.elapsed);if(match.phase==='PRIMER TIEMPO'&&match.elapsed>=HALF_TIME_SECONDS(match)){clearInterval(timerId);ticking=false;match.phase='DESCANSO';matchesService.setActive(match);void matchesService.save(match).catch(error=>alert(`No se pudo guardar el descanso: ${error.message}`));render();return}if(match.elapsed%10===0){matchesService.setActive(match);void matchesService.save(match).catch(()=>{})}},1000)}
